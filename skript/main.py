...
def check_database_existence(timestream_client, database_name):
    try:
        ...
        logging.info("Database [%s] does not exist.", database_name)
        return False
...
def create_database(timestream_client, database_name):
    try:
        ...
        logging.info("Database [%s] successfully created.", database_name)
        return True
...
def check_table_existence(timestream_client, database_name, table_name):
    try:
        ...
        logging.info("Table [%s] in database [%s] already exists.", table_name, database_name)
        return True
    except timestream_client.exceptions.ResourceNotFoundException:
        logging.info("Table [%s] in database [%s] does not exist.", table_name, database_name)
        return False
    except Exception as e:
...
def create_table(timestream_client, database_name, table_name):
    ...
    try:
        ...
        logging.info("Table [%s] successfully created.", table_name)
        return True
    except timestream_client.exceptions.ConflictException:
        logging.info("Table [%s] exists in database [%s]. Skipping table creation.", table_name, database_name)
        return False
    except Exception as e:
...
def find_sensor_id():
    ...
    sensor_id = next((os.path.basename(sensor_path) for sensor_path in sensor_list if os.path.basename(sensor_path).startswith("28-")), None)
    return sensor_id
                ...
                if temperature != 0 and temperature != 85:
                    ...
                    try:
                        ...
                        record = {
                            ...
                            'Time': str(int(current_timestamp.timestamp() * 1000)),  # Convert to milliseconds
                        }

                        # Write the record to Timestream
                        timestream_client.write_records(DatabaseName=database_name, TableName=table_name, Records=[record])
                        print(f"Sensor {sensor_id}: Temperature: {temperature:.2f}°C - Written to Timestream")
